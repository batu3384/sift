...
allow_only_in() {
  ...
  local matches
  matches="$(rg -n "$pattern" internal cmd || true)"
  if [[ -z "$matches" ]]; then
    return 0
  fi
...
allow_only_in_any 'exec\.Command(Context)?' 'internal/platform/current_darwin.go' 'internal/platform/current_darwin_diagnostics.go' 'internal/platform/admin_session.go' 'internal/engine/native.go' 'internal/engine/health_darwin.go' 'internal/tui/open_darwin.go' 'internal/tui/open_windows.go' 'internal/tui/preflight.go'
...
reject_any 'rm -rf'
reject_any 'Remove-Item\s+-Recurse\s+-Force'
